...
if (isset($_GET['delete_id'])) {
    ...
    $deleteGradesQuery = $pdo->prepare("DELETE FROM grades WHERE Student_ID = :id");
    $deleteGradesQuery->bindParam(':id', $delete_id, PDO::PARAM_INT);
    $deleteGradesQuery->execute();

    $deleteQuery = $pdo->prepare("DELETE FROM students WHERE Student_ID = :id");
                                    ...
                                    <tbody>
                                        <?php foreach ($students as $student) : ?>
                                            <tr>
                                                ...
                                                <td><?= htmlspecialchars($student['Department_ID'] ?? '', ENT_QUOTES, 'UTF-8') ?></td>
                                                <td><?= htmlspecialchars($student['Email']) ?></td>
                                                <td><?= htmlspecialchars($student['Current_Semester']) ?></td>
                                                <td><?= date("d-m-Y", strtotime(htmlspecialchars($student['Date_Of_Birth']))) ?></td>
                                                <td>
                                                    <a href="NewEditStudent.php?Student_ID=<?= htmlspecialchars($student['Student_ID']) ?>">
                                                        <button type="button" class="action-button">Edit</button>
                                                    </a>
                                                    <a href="NewStudentAccess.php?delete_id=<?= htmlspecialchars($student['Student_ID'], ENT_QUOTES, 'UTF-8') ?>" onclick="return confirm('Are you sure you want to delete this student?')">
