...
// Check if Instructor_ID is provided in the query string
if (isset($_GET['Instructor_ID'])) {
    $instructorId = $_GET['Instructor_ID'];

    // Fetch the instructor details from the database
    if ($stmt = $conn->prepare("SELECT * FROM instructors WHERE Instructor_ID = ?")) {
        $stmt->bind_param("i", $instructorId);
        $stmt->execute();
        $result = $stmt->get_result();
        $instructor = $result->fetch_assoc();

        // If instructor not found, handle it accordingly
        if (!$instructor) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $instructorId = $_POST['Instructor_ID']; // Added to get Instructor_ID from POST

    if ($stmt = $conn->prepare("UPDATE instructors SET First_Name = ?, Middle_Name = ?, Last_Name = ?, Gender = ?, Department_ID = ?, Contact_Info = ? WHERE Instructor_ID = ?")) {
        $stmt->bind_param("ssssisi", $firstName, $middleName, $lastName, $gender, $departmentId, $contactInfo, $instructorId);


        // Execute the statement
        if ($stmt->execute()) {
            // echo "Instructor details updated successfully!";
            // Optionally, you can redirect to another page or refresh the same page
...
<style>
    ...
    * {
    ...
    body.dark-mode .save-changes-btn {
        ...
        /* Green background for dark mode */
        color: black;
        /* Black text for dark mode */
    }

    /* Light Mode Styles */
    body.light-mode .save-changes-btn {
    ...
    .save-changes-btn:hover {
        ...
        /* Darker green for hover in dark mode */
    }

    /* Light mode hover effect */
    body.light-mode .save-changes-btn:hover {
        background-color: #0099cc;
        /* Darker light blue for hover in light mode */
    }
    ...
    .save-changes-btn:disabled {
        ...
        color: #666666;
        /* Dark grey text for disabled state */
        cursor: not-allowed;
        /* Not-allowed cursor */
    }
</style>
                            ...
                            <div class="table-container">
                                ...
                                <div class="form-container">
                                    ...
                                    <form method="POST" action="">
                                        <input type="hidden" name="Instructor_ID" value="<?= isset($instructor['Instructor_ID']) ? htmlspecialchars($instructor['Instructor_ID']) : '' ?>" />

                                        <p><strong>First Name:</strong>
                                            <input type="text" name="First_Name" value="<?= isset($instructor['First_Name']) ? htmlspecialchars($instructor['First_Name']) : '' ?>" required />
                                        </p>

                                        <p><strong>Middle Name:</strong>
                                            <input type="text" name="Middle_Name" value="<?= isset($instructor['Middle_Name']) ? htmlspecialchars($instructor['Middle_Name']) : '' ?>" required />
                                        </p>

                                        <p><strong>Last Name:</strong>
                                            <input type="text" name="Last_Name" value="<?= isset($instructor['Last_Name']) ? htmlspecialchars($instructor['Last_Name']) : '' ?>" required />
                                        </p>

                                        <p><strong>Gender:</strong>
                                            <input type="text" name="Gender" value="<?= isset($instructor['Gender']) ? htmlspecialchars($instructor['Gender']) : '' ?>" required />
                                        </p>

                                        <p><strong>Contact Info:</strong>
                                            <input type="text" name="Contact_Info" value="<?= isset($instructor['Contact_Info']) ? htmlspecialchars($instructor['Contact_Info']) : '' ?>" required />
                                        </p>

                                        <p><strong>Department:</strong>
                                            <input type="text" name="Department_ID" value="<?= isset($instructor['Department_ID']) ? htmlspecialchars($instructor['Department_ID']) : '' ?>" />
                                        </p>
