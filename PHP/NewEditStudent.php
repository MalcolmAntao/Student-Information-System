...
if (isset($_GET['Student_ID'])) {
    ...
    if ($stmt = $conn->prepare("SELECT * FROM students WHERE Student_ID = ?")) {
        $stmt->bind_param("i", $studentId);
        $stmt->execute();
        $result = $stmt->get_result();
        $student = $result->fetch_assoc();

        // If student not found, handle it accordingly
        if (!$student) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt = $conn->prepare("UPDATE students SET First_Name = ?, Gender = ?, Roll_No = ?, University_No = ?, Department_ID = ?, PhoneNo = ?, Major = ?, Current_Semester = ?, Email = ?, Date_Of_Birth = ? WHERE Student_ID = ?")) {
        $stmt->bind_param("ssssisssssi", $firstName, $gender, $rollNo, $universityNo, $departmentId, $phoneNo, $major, $currentSemester, $email, $dateOfBirth, $studentId);
...
<style>
    ...
    * {
    ...
    .add-more-btn {
        position: absolute;
        top: 15px;
        /* Adjusts the vertical alignment relative to the block */
        right: 10px;
        /* Adjusts the horizontal alignment to place it near the right edge */
        background-color: #1e1e1e;
        color: white;
        border: none;
        cursor: pointer;
    ...
    button {
        ...
        /* Black background for edit button */
        color: white;
        /* White text */
        padding: 10px 20px;
        /* Padding for button */
