...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $plainPassword = $First_Name . $Last_Name . '123';

    // Hash the password using bcrypt
    $hashedPassword = password_hash($plainPassword, PASSWORD_BCRYPT);

    // Insert data into the instructors table
    $sql = "INSERT INTO instructors (First_Name, Middle_Name, Last_Name, Gender, Contact_Info, Department_ID) 
            VALUES (:First_Name, :Middle_Name, :Last_Name, :Gender, :Contact_Info, :Department_ID)";
    ...
    // Execute the statement for instructors
    if ($stmt->execute()) {
        // Retrieve the Role_ID for "teacher" from roles table
        $roleQuery = "SELECT Role_ID FROM roles WHERE Role_Name = 'teacher'";
        $roleStmt = $pdo->query($roleQuery);
        $roleResult = $roleStmt->fetch(PDO::FETCH_ASSOC);
        $Role_ID = $roleResult['Role_ID'];

        // Insert data into the users table
        $userSql = "INSERT INTO users (Email, Password, Role_ID) VALUES (:Contact_Info, :Password, :Role_ID)";
        $userStmt = $pdo->prepare($userSql);

        // Bind the parameters for users table
        $userStmt->bindParam(':Contact_Info', $Contact_Info);
        $userStmt->bindParam(':Password', $hashedPassword); // Store the hashed password
        $userStmt->bindParam(':Role_ID', $Role_ID);

        // Execute the statement for users table
        if ($userStmt->execute()) {
            // Redirect to the same page to prevent resubmission on refresh and show success message
            header("Location: NewEntry(T).php?success=1&password=" . urlencode($plainPassword));
            exit(); // Important to stop further execution after redirect
        } else {
            // Error handling for user table insertion
            $errorInfo = $userStmt->errorInfo();
            echo "Error saving user data: " . $errorInfo[2];
        }
    } else {
        // Error handling for instructors table insertion
        $errorInfo = $stmt->errorInfo();
        echo "Error saving instructor data: " . $errorInfo[2];
    }
}
?>
<!DOCTYPE html>
...
<style>
    ...
    * {
    ...
    form input::placeholder {
        color: #c0c0c0;
        /* Light gray color for the placeholder in dark mode */
    }
    ...
    body.light-mode form input::placeholder {
        color: #1e1e1e;
        /* Darker gray color for the placeholder in light mode */
    }
            ...
            <div class="column-2">
                ...
                <div class="courses gradient-card">
                    <div class="course-card ">
                        ...
                        <div class="course-marks gradient-bg-hover" style="align-items: center;">
                            <div class="table-container" style="align-items: center;">
                                <!-- Form Section -->
                                <div class="form-container" style="align-items: center;">
                                    <form id="teacher-form" action="NewEntry(T).php" method="POST" style="align-items: center;">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="date" id="Enrollment_Date" name="Enrollment_Date" placeholder="Enrollment Date" required>
                                        </div>
                                        <!-- Add Save Button -->
...
<script>
    ...
    toggleButton.addEventListener('click', function() {
        ...
        // Format the date as yyyy-mm-dd (required for date input value)
