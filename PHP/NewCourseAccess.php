...
try {
    ...
    if (isset($_GET['delete_id'])) {
        ...
        try {
            ...
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':course_id', $deleteId, PDO::PARAM_INT);

            // Execute the delete query
            if ($stmt->execute()) {
                // If deletion was successful, redirect back to the CourseAccess page
                header("Location: CourseAccess.php");
                exit();
            } else {
    ...
    // Fetch courses (your existing code to display courses)
    try {
        $sql = "SELECT * FROM courses";
        $stmt = $pdo->prepare($sql);
        $stmt->execute();
...
<style>
    ...
    * {
    ...
    .action-button {
        ...
        font-weight: bold;
        color: white;
        background-color: var(--course-card-dark-gradient);
        /* Green background */
        border: 2px solid transparent;
        border-radius: 25px;
        cursor: pointer;
                ...
                <div class="courses gradient-card">
                    <div class="course-card ">
                        ...
                        <div class="course-marks gradient-bg-hover">
                                ...
                                <table class="course-table">
                                    ...
                                    <tbody>
                                        <?php foreach ($courses as $courses): ?>
                                            <tr>
                                                <td><?= htmlspecialchars($courses['Course_ID']) ?></td>
                                                <td><?= htmlspecialchars($courses['course_code']) ?></td>
                                                <td><?= htmlspecialchars($courses['CourseName']) ?></td>
                                                <td class="course-content" data-full-content="<?= htmlspecialchars($courses['Description']) ?>">
                                                    <?= htmlspecialchars(substr($courses['Description'], 0, 25)) ?>..
                                                </td>
                                                <td><?= htmlspecialchars($courses['Credits']) ?></td>
                                                <td><?= htmlspecialchars($courses['Department_ID']) ?></td>
                                                <td><?= htmlspecialchars($courses['Semester']) ?></td>
                                                <td><?= htmlspecialchars($courses['Enrollment_Type_ID']) ?></td>
                                                <td>
                                                    <div class="button-section">
                                                        <a href="NewCourseAccess.php?delete_id=<?= htmlspecialchars($courses['Course_ID'], ENT_QUOTES, 'UTF-8') ?>" onclick="return confirm('Are you sure you want to delete this course?')">
                                                            <button type="button" class="action-button delete-button">Delete Course</button>
                                                        </a>
                                                    </div>
                                                </td>
                                            </tr>
                            ...
                            <!-- Buttons Section -->
                            <div class="button-section">
                                <a href="../PHP/NewCourse2.php">
                                    <button type="button" class="action-button">New Course</button>
                                </a>
                            </div>
                        </div>
                    </div>
